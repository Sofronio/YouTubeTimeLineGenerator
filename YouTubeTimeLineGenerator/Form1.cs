    ...
    public partial class Form1 : Form
    {
        ...
        private void button_processVTT_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                button_autoSegment.Enabled = true;
            }
            catch
            {
        ...
        private void textBox_autoSegmentDataSourceFilePath_DragOver(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                e.Effect = DragDropEffects.Copy;
        ...
        private void textBox_autoSegmentDataSourceFilePath_DragDrop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                {
                    textBox_autoSegmentDataSourceFilePath.Text = paths[0];
                }
            }
        }

        private void button_autoSegment_Click(object sender, EventArgs e)
        {
            // Check parameters

            string rawDataSourceJsonFilePath = textBox_autoSegmentDataSourceFilePath.Text;

            if (!rawDataSourceJsonFilePath.Contains("\\") && !rawDataSourceJsonFilePath.Contains("/"))
            {
                MessageBox.Show($"Please drag-and-drop a json file or input the file path first",
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (!File.Exists(rawDataSourceJsonFilePath))
            {
                MessageBox.Show($"The specified json file does not existed:{Environment.NewLine}{rawDataSourceJsonFilePath}",
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            int sampleValueAudibleThreshold = trackBar_autoSegmentThreshold.Value;

            // Read the specified raw data source file generated by BeatShowCore

            string rawDataSourceJson = File.ReadAllText(rawDataSourceJsonFilePath, Encoding.UTF8);

            JObject rawDataSource;
            try
            {
                rawDataSource = JObject.Parse(rawDataSourceJson);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error occurred when parsing the specified file:{Environment.NewLine}{ex.Message}",
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            List<JToken> dataSourceList = rawDataSource["dataSourceList"]?.Children().ToList();
            ...
            foreach (JToken dataSource in dataSourceList)
            {
                if ((dataSource["type"]?.ToString() == "analog")
                        && (dataSource["name"]?.ToString() == "vocal"))
                {
                    List<JToken> valueTokens = dataSource["values"]?.ToList();
                    if (valueTokens == null)
                    {
                        continue;
                    }

                    foreach (JToken valueToken in valueTokens)
                    {
                        sampleValues.Add(valueToken.Value<int>());
                    }
            ...
            {
                MessageBox.Show($"Did not get any vocal sample values",
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            List<int> segmentationPointListInMs = new List<int>();

            bool? lastSampleIsAudible = null;
            int? currentSilentRegionBeginIndex = null;
            for (int i = 0; i < sampleValues.Count; i++)
            {
                bool currentSampleIsAudible = (sampleValues[i] >= sampleValueAudibleThreshold);

                if (currentSampleIsAudible != lastSampleIsAudible)
                {
                    // silent -> audible, or audible -> slient

                    if (currentSampleIsAudible)
                    {
                        // silent -> audible

                        if (currentSilentRegionBeginIndex != null)
                        {
                            // Convert sample index (sample rate 25Hz) to ms
                            int silentRegionBeginTimeInMs = (int)currentSilentRegionBeginIndex * (1000 / 25);
                            int silentRegionEndTimeInMs = (i - 1) * (1000 / 25);

                            if ((silentRegionEndTimeInMs - silentRegionBeginTimeInMs) >= 300)
                            {
                                segmentationPointListInMs.Add(((silentRegionBeginTimeInMs + silentRegionEndTimeInMs) / 2));
                            }

                            currentSilentRegionBeginIndex = null;
                        }
                    }
                    else
                    {
                        ...
                        currentSilentRegionBeginIndex = i;
                    }
                }

                lastSampleIsAudible = currentSampleIsAudible;
            }
            ...
            int? lastSegmentationPointIndex = null;
            subtitle lastSubtitle = null;
            foreach (subtitle subtitleWord in mySubtitle)
            {
                int subtitleWordTimeInMs = (timeToMs(subtitleWord.WordStart) + 150);

                int foundSegmentationPointIndex = -1;
                for (int i = 0; i < segmentationPointListInMs.Count - 1; i++)
                {
                    if ((subtitleWordTimeInMs >= segmentationPointListInMs[i]) && (subtitleWordTimeInMs <= segmentationPointListInMs[i + 1]))
                    {
                        foundSegmentationPointIndex = i;
                        break;
                    }
                }

                if (foundSegmentationPointIndex != lastSegmentationPointIndex)
                {
                    if (lastSubtitle != null)
                    {
                        lastSubtitle.Selected = true;
                    }

                    lastSegmentationPointIndex = foundSegmentationPointIndex;
                }

                lastSubtitle = subtitleWord;
            }
            ...
            foreach (var ms in mySubtitle)
            {
                if (ms.Selected)
                {
                    richTextBox_Words.SelectionStart = ms.Position + ms.Content.Length;
                    richTextBox_Words.SelectionLength = 1;
                    richTextBox_Words.SelectionBackColor = Color.Red;
                    richTextBox_Words.SelectionLength = 0;
                }
        ...
        {
            TimeSpan timeSpan = TimeSpan.ParseExact(time, "hh\\:mm\\:ss\\.fff", System.Globalization.CultureInfo.InvariantCulture);
            int ms = (int)timeSpan.TotalMilliseconds;
